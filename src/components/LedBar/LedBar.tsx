...
import { LED_SPACING_MM, LED_START_X_MM, TRACE_WIDTHS } from "../../utils/constants"
...
export function LedBar() {
  return (
    <>
      <A_74HC595D_118 name="U6" pcbX={21} pcbY={3} pcbRotation={90} />
      <A_74HC595D_118 name="U7" pcbX={33} pcbY={3} pcbRotation={90} />
      <A_74HC595D_118 name="U8" pcbX={45} pcbY={3} pcbRotation={90} />
      <capacitor name="C16" capacitance="100nF" footprint="0603" supplierPartNumbers={supplierPartNumbers.capacitor0603_100n} pcbX={21} pcbY={11} />
      <capacitor name="C17" capacitance="100nF" footprint="0603" supplierPartNumbers={supplierPartNumbers.capacitor0603_100n} pcbX={33} pcbY={11} />
      <capacitor name="C18" capacitance="100nF" footprint="0603" supplierPartNumbers={supplierPartNumbers.capacitor0603_100n} pcbX={45} pcbY={11} />
      ...
      {ledXs.map((_, index) => (
        <Fragment key={`rled-vcc-${index + 1}`}>
          <trace from={`RLED${index + 1}.pin1`} to={nets.v3v3} width={TRACE_WIDTHS.led} />
        </Fragment>
      ))}
      {ledXs.map((_, index) => (
        <Fragment key={`driver-rled-${index + 1}`}>
          <trace from={ledSources[index]} to={`RLED${index + 1}.pin2`} width={TRACE_WIDTHS.led} />
        </Fragment>
      ))}
      {ledXs.map((_, index) => (
        <Fragment key={`rled-led-${index + 1}`}>
          <trace from={`RLED${index + 1}.pin2`} to={`LED${index + 1}.pin1`} width={TRACE_WIDTHS.led} />
        </Fragment>
      ))}
      {ledXs.map((_, index) => (
        <Fragment key={`led-gnd-${index + 1}`}>
          <trace from={`LED${index + 1}.pin2`} to={nets.gnd} width={TRACE_WIDTHS.led} />
        </Fragment>
      ))}

      <silkscreentext pcbX={-20} pcbY={20.7} text="LEFT 10 SEG" fontSize="1mm" />
      <silkscreentext pcbX={25} pcbY={20.7} text="RIGHT 10 SEG" fontSize="1mm" />
    </>
  )
}
